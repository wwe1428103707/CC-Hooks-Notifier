...
在托盘图标右键菜单中点击"Dashboard"打开 WebView2 窗口：

- **仪表盘**标签 — 服务状态、通知/子代理/任务计数、最近事件
- **事件日志**标签 — 完整事件历史，包含时间戳、级别和内容
